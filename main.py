...
import pygame
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QVBoxLayout, QLineEdit, QPushButton, QMessageBox, QDialog, \
	QListWidget
...
def update_score(username, new_score):
	try:
		...
		# Обновляем score для заданного username
		cursor.execute("""
            UPDATE users
            SET score = ?
            WHERE username = ?
...
# Вызываем функцию создания базы данных перед использованием
create_database()
...
class LoginApp(QWidget):
	def __init__(self):
		...
		self.setWindowTitle("Login and Top 10 App")
	...
	def register_user(self):
		...
		if self.register(username, password):
			...
			QMessageBox.warning(self, "Registration Failed", "User with this username already exists")

	def authenticate(self, username, password):
		# Здесь вы можете подключиться к базе данных и проверить логин и пароль
		# Замените следующий код на ваш код для работы с базой данных SQLite3
		conn = sqlite3.connect("users.db")
		cursor = conn.cursor()
	...
	def show_top_ten(self, current_user):
		conn = sqlite3.connect("users.db")
		cursor = conn.cursor()
		...
		top_ten_dialog = QDialog(self)
		top_ten_dialog.setWindowTitle("Top 10 Users")

		layout = QVBoxLayout()

		top_ten_label = QLabel("Top 10 Users:")
		layout.addWidget(top_ten_label)

		global list_widget
		list_widget = QListWidget()
		for i, user in enumerate(top_ten_users, start=1):
			list_widget.addItem(f"{i}. {user[1]} - {user[3]}")
		layout.addWidget(list_widget)

		current_user_label = QLabel(f"Your username: {current_user}")
		...
		play_button = QPushButton("Update")
		play_button.clicked.connect(self.update_top)
		layout.addWidget(play_button)

		top_ten_dialog.setLayout(layout)
		top_ten_dialog.exec_()
	...
	def update_top(self):
		...
		conn.close()
		global list_widget
		list_widget.clear()  # Очищаем список перед обновлением
		...
		class Topscore:
			...
			def top_score(self, score):
				if score > self.high_score:
					...
					update_score(self.nickname, score)
