...
class LoginApp(QWidget):
	...
	# Функция для обработки нажатия кнопки "Login"
	def login(self):
	...
	# Функция для обработки нажатия кнопки "Register"
	def register_user(self):
		...
		if self.register(username, password):
			...
			QMessageBox.warning(
				self, "Registration Failed", "User with this username already exists"
			)
	...
	# Функция для отображения топ-10 пользователей
	...
	# Функция для обновления списка топ-10 пользователей
	def update_top(self):
		conn = sqlite3.connect("users.db")
		cursor = conn.cursor()
		cursor.execute("SELECT * FROM users ORDER BY score DESC LIMIT 10")
